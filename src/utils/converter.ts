...
const parseTokenAmount = (amount: BigSource, decimals: number = 0): string => Big(amount).times(Big(10).pow(decimals)).toFixed();
...
const toTGas = (amount: string): string => formatTokenAmount(amount, 12); 
const toGas = (amount: string): string => parseTokenAmount(amount, 12);

// yocto -> NEAR
const toNEAR = (amount: string | number): string => formatTokenAmount(amount.toString(), 24);
// NEAR -> yocto
const toYocto = (amount: string | number): string => parseTokenAmount(amount.toString(), 24);

const convert = (amount: string | number, unit: string | null = "unknown", decimals?: number): number | string => {
...
const unitToDecimals = (unit: string | null = "unknown"): number | undefined => ({
        NEAR: 24,
        yocto: 0,
        Tgas: 12,
        gas: 0
