...
import { JobData, JobModel } from "./job.model";
import { IJobService, JobService } from "./job.service";
import "./job.ui.scss";

const jobStatusIcons: Record<JobData["status"], string> = {
    Inactive: "🟡",
    Expired: "🔴",
    Deleted: "🔴",
    Active: "🟢",
...
const JobDisplayStatus = ({ status, job }: Pick<JobData, "status" | "job">) => {
    /*
...
export const jobAsTableRow = ({ raw: jobRaw, normalized: jobNormalized }: JobModel) => {
    const { id, status, job } = jobNormalized;
    return {
        content: [
            ...
            <div className="Job-multicalls">
                <button
                    ...
                    onClick={(e: MouseEvent) => {
                        navigator.clipboard.writeText(JSON.stringify(jobRaw.job.multicalls));
